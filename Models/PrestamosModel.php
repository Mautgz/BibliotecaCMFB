...
class PrestamosModel extends Query
{
    ...
    {
        $sql = "SELECT e.id, e.codigo, e.nombre, e.año, l.id, l.titulo, p.id, p.id_estudiante, p.id_libro, p.fecha_prestamo, p.fecha_devolucion, p.cantidad, p.observacion, p.estado FROM estudiante e INNER JOIN libro l INNER JOIN prestamo p ON p.id_estudiante = e.id WHERE p.id_libro = l.id AND p.id = $id_prestamo";
        $res = $this->select($sql);
        return $res;
    }
    public function buscarPrestamosPorFecha($fecha_inicio, $fecha_fin)
    {
        $sql = "SELECT e.id, e.nombre, l.titulo, p.fecha_prestamo, p.fecha_devolucion, p.cantidad, p.observacion, p.estado 
                FROM estudiante e 
                INNER JOIN prestamo p ON p.id_estudiante = e.id 
                INNER JOIN libro l ON p.id_libro = l.id 
                WHERE p.fecha_prestamo BETWEEN ? AND ?";
        $datos = array($fecha_inicio, $fecha_fin);
        return $this->selectAll($sql, $datos);
    }
    public function getTotalPrestamosActivos()
    {
        $sql = "SELECT COUNT(*) as total FROM prestamo WHERE estado = 1";
        $data = $this->select($sql);
        return $data['total'];
    }
    public function getTotalPrestamosDevueltos()
    {
        $sql = "SELECT COUNT(*) as total FROM prestamo WHERE estado = 0";
        $data = $this->select($sql);
        return $data['total'];
    }
}
